...
import type { IdeaNote, SectorsPayload, SleeveRow } from "./types";
...
export function App() {
  ...
  const load = useCallback(async (bust = false) => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/sectors${bust ? `?t=${Date.now()}` : ""}`);
      const json = await res.json();
      if (!res.ok) throw new Error(json.detail || json.error || "Failed to load");
      setData(json);

      const leader = json.dayBoard?.[0]?.name as string | undefined;
    ...
    <div className="page">
      ...
      <main>
        <section className="hero">
          ...
          <p className="lede">
            Sector heat, theme baskets, and liquidity notes — without the repeated alerts.
          </p>
        </section>
